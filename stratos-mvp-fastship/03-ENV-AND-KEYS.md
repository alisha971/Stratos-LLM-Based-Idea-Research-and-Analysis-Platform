...
| `GROQ_API_KEY_ALISHA` | Yes | LLM inference. Get it free at [console.groq.com/keys](https://console.groq.com/keys). Powers clarification, outline, research queries, and section writing. |
| `GROQ_API_KEY_ENCRIL` | Yes | A second Groq key on a *separate* account. `app/llm/client.py` uses the two as a fallback pool — one primary, the other on rate-limit/API failure — roughly doubling the effective daily token quota. |
...
| `PRODUCT_HUNT_TOKEN` | Optional | One of two discovery sources for the competitor worker (the other, Hacker News, needs no key). Free, non-expiring: Product Hunt account → [API Dashboard](https://api.producthunt.com/v2/oauth/applications) → "Add an application" → copy the `developer_token`. Leave unset to run on Hacker News (Show HN) alone — the worker degrades silently, never fails. |
| `COMPETITOR_MAX` | Optional | Max verified competitors profiled per report; default `7`. |
